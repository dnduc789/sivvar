...
public class InCallActivity extends FragmentActivity implements OnClickListener {
	...
	private TextView pause, hangUp, dialer, video, micro, speaker, options, addCall, transfer, conference, statistics;
	...
	private boolean isSpeakerEnabled = false, isMicMuted = false, isStatisticsOn = false, isTransferAllowed,
			isAnimationDisabled;
	private ViewGroup mControlsLayout;
	private Numpad numpad;
	private int cameraNumber;
	private Animation slideOutLeftToRight, slideInRightToLeft, slideInBottomToTop, slideInTopToBottom,
			slideOutBottomToTop, slideOutTopToBottom;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (findViewById(R.id.fragmentContainer) != null) {
			...
			getSupportFragmentManager().beginTransaction().add(R.id.fragmentContainer, callFragment)
...
//		enableAndRefreshInCallActions();
	...
	private void initUI() {
		...
		mPassword = (Button) findViewById(R.id.PasswordVisible);
		mPassword.setOnClickListener(this);
		// mAddress.setTransformationMethod(new
	...
	private void refreshInCallActions() {
		...
		if (LinphoneManager.getLc().getCallsNb() > 1) {
			...
			List<LinphoneCall> pausedCalls = LinphoneUtils.getCallsInState(LinphoneManager.getLc(),
	...
	private void enableAndRefreshInCallActions() {
		...
		options.setEnabled(!getResources().getBoolean(R.bool.disable_options_in_call)
	...
	public void onClick(View v) {
		...
		if (id == R.id.video) {
			...
			if (mAddress.length() != 0) {
				...
				if (Integer.parseInt((String) mPassword.getTag()) == 0) { // hide
					mPassword.setBackgroundResource(R.drawable.show);
					mPassword.setTag("1");
					mAddress.setTransformationMethod(new AsteriskPasswordTransformationMethod());
					mAddress.setSelection(mAddress.length());
				} else {
					mPassword.setBackgroundResource(R.drawable.hide);
					mPassword.setTag("0");
					mAddress.setTransformationMethod(HideReturnsTransformationMethod.getInstance());
					mAddress.setSelection(mAddress.length());
				}
			}
	...
	private void toggleStatistics() {
		...
		if (isStatisticsOn) {
			status.open();
			statistics.setBackgroundResource(R.drawable.statistics_on);
		} else {
			status.close();
			statistics.setBackgroundResource(R.drawable.statistics_off);
		}
	}
	...
	private void hideOrDisplayNumpad() {
		...
		LinearLayout address = (LinearLayout) findViewById(R.id.Addresstext);
	...
	private void hideOrDisplayCallOptions() {
		boolean isOrientationLandscape = getResources()
	...
	private void displayCall(Resources resources, LinphoneCall call, int index) {
		...
		try {
			...
			LinphoneCallLog log = call.getCallLog();
			if (log.getDirection() == CallDirection.Incoming) {
				address = log.getFrom();
			} else {
				address = log.getTo();
			}
			lAddress.setDisplayName(address.getDisplayName());
		} catch (LinphoneCoreException e) {
		...
		Bitmap picture = AppUtil.getAvatar(lAddress.getUserName());
		displayOrHideContactPicture(imageView, picture, false);
	...
	private void displayOrHideContactPicture(LinearLayout callView, Uri pictureUri, boolean hide) {
		...
		if (pictureUri != null) {
			LinphoneUtils.setImagePictureFromUri(callView.getContext(), contactPicture.getView(),
					Uri.parse(pictureUri.toString()), R.drawable.unknown_small);
		}
		callView.setVisibility(hide ? View.GONE : View.VISIBLE);
	}

	private void displayOrHideContactPicture(LinearLayout callView, Bitmap picture, boolean hide) {
		AvatarWithShadow contactPicture = (AvatarWithShadow) callView.findViewById(R.id.contactPicture);
		if (picture != null) {
			LinphoneUtils.setImagePictureFromBitmap(callView.getContext(), contactPicture.getView(), picture,
